...
/** 한국 시각 전체 문자열 (날짜/시간 질문에 그대로 답할 때 사용) */
function getCurrentKstDateTimeString(clientTimeIso?: string): string {
  const now = clientTimeIso ? new Date(clientTimeIso) : new Date();
...
예: 할아버지/할머니에게 "아침 식사는 하셨나요?", "점심 드셨나요?", "오늘 날씨가 맑은데 산책 어떠세요?" 등. (본인 소개 포함)`;
    ...
    // 날짜/시간 질문은 서버 시각으로 바로 답 (Gemini 미호출)
    if (
      !audio?.data &&
      lastUserMessage &&
      isDateTimeQuestion(lastUserMessage)
    ) {
      const timeStr = getCurrentKstDateTimeString(clientContext?.currentTime);
