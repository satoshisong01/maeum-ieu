...
const NAME_STOPWORDS = new Set([
  ...NAME_STOPWORDS_BASE,
  // 호칭·가족 관계 (false positive 차단, 이 파일 전용)
  "민지", "안사람", "아내", "남편", "엄마", "아빠", "어머니", "아버지",
  "손자", "손녀", "큰아들", "둘째", "막내", "장남", "차남", "장녀", "차녀",
  // 일반 명사·대명사
  "오늘", "어제", "그때", "이때", "여기", "거기", "정말", "민지가", "성함", "이름",
  // 조사+종결어미 잔여
 ...
 */
// 단글자 행정 접미사(시/구/동/읍/면)는 존댓말·연결어미와 충돌("편하시군요"→"편하", "먹어주면"→"먹어주")
// — 후보 전원이 쓰레기·미근거가 되어 wholesale 교체를 오발동시킴(2026-06-11 김치 사이클, 死 정규식 클래스).
// 장소 인정 조건: 어말(비한글/끝) 또는 처소격 조사(에/에서/으로/로/까지/부터) 직결. 연결어미 '면'은 어말 빈도가 높아 처소격 필수.
const FACT_NOUN_PATTERN = /([가-힣]{2,4})(?:이는|이가|이도|이를|이에게|이한테|아드님|따님|손주|손자|손녀|동네|병원|약국|할머니|할아버지|[시구동읍](?=$|[^가-힣]|에서|에|으로|로|까지|부터)|면(?=에서|에|으로|로|까지|부터))/g;
...
function extractNameCandidates(text: string): Set<string> {
  ...
  for (let pi = 0; pi < NAME_CONTEXT_PATTERNS.length; pi++) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      // 연결어미·부사·관형형으로 끝나는 후보는 이름일 수 없음 — "담가서 아드님들이"의
