...
const SOFT_SIGNAL = /약|수면제|죽|자살|숨|가슴|쓰러|넘어|미끄러|피[가\s]|어지|힘들|외로|혼자|그만|살기|사라지|따라가|잠들|못\s*깨|깨어나|영원|먼저\s*[가갈]|확\s*[가갈]|모아|털어|들이[켜키]|삼[키켜켰]|먹어버|먹어불|두\s*번|두\s*알|또\s*먹|섞어|헷갈|착각|움큼|한\s*통|식은\s*땀|손\s*(?:이|을)?\s*떨|떨려|떨린|핑\s*도|기운\s*(?:이|을)?\s*(?:없|빠)|기력|당뇨|혈압|토[하해]|구역|메스|마비|경련|발작|데[였여어]|화상|부러|골절|삐[었어]|쥐\s*나|저[려린릿]/;
