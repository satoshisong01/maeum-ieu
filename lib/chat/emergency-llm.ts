...
export const SOFT_SIGNAL = /약|수면제|죽|자살|목\s*[매졸]|매달|올가미|끈\s|뛰어내|투신|번개탄|연탄|목숨|세상\s*[뜨떠]|저\s*세상|먼저\s*[가갈]|따라\s*[가갈]|곁으로|조용히\s*[가갈]|가\s*[뿌뿔삐]|살\s*맛|폐[\s가-힣]{0,4}끼|짐\s*(?:만|이|덩)|그만\s*살|살기\s*싫|사라지|없어졌으면|없어지고|눈\s*감|눈\s*안\s*[뜨떠]|안\s*깨|깨어나|영원|모[아았]|털어|들이[켜키]|삼[키켜켰]|먹어버|먹어불|입에\s*[넣털]|두\s*번|두\s*알|세\s*알|한\s*알|또\s*먹|섞어|헷갈|착각|움큼|한\s*통|숨|가슴|쓰러|넘어|미끄러|피[가\s]|출혈|어지|핑\s*도|식은\s*땀|손\s*(?:이|을)?\s*떨|떨려|떨린|마비|경련|발작|의식|안\s*올라가|헛나|어눌|혀\s*꼬|부들부들|벌벌\s*떨|덜덜\s*떨|거품\s*물|까무러|까무라|기절|혼절|삐[뚤뚜]|비뚤|감각\s*(?:이|도)?\s*(?:없|둔)|(?:한|왼|오른|반)\s*[쪽짝편]\s*(?:팔|다리|손|발|몸|얼굴|눈|입)|반신\s*마|기운\s*(?:이|을)?\s*(?:없|빠)|기력|당뇨|혈압|토[하해]|구역|메스|데[였여어]|화상|끓는|뜨거운\s*물|물집|살갗|부러|골절|삐[었어]|쥐\s*나|저[려린릿]|열이\s*(?:나|많|펄)|힘들|외로|혼자|괴로|우울|허전|보고\s*싶|그립|눈물/;
...
판정 원칙: 확실치 않으면 보수적으로 — 진짜 위기를 놓치는 것보다 약하게라도 잡는 게 낫습니다. 단, 단순 복약 질문·부정문·평범한 일상, 그리고 "보고 싶어 죽겠다 / 좋아 죽겠다 / 배고파 죽겠다 / 보고 싶어 죽겠어"처럼 그리움·강조의 관용 표현(실제 자살 의도 아님)은 반드시 none(과잉경보 금지). 자살은 스스로 목숨을 끊으려는 실제 의도가 있을 때만. 또한 "잠깐/가끔/조금/천천히 하면 괜찮은" 정도의 경미하고 일시적인 신체 증상은 none — medical_acute는 갑작스럽고 심하거나 지속되는 경우만(예: "휙 일어나면 잠깐 핑 도는 정도"·"가끔 무릎이 쑤셔"는 응급 아님).
