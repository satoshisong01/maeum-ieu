...
export const SOFT_SIGNAL = /(?<![가-힣])약(?!속|국|간|혼|소|점|장|사)|수면제|살려\s*[주줘달]|사람\s*살려|죽|자살|목(?:이라도|을|에)?\s*[매졸]|매달|올가미|끈\s|뛰어내|투신|번개탄|연탄|손목|그어|칼\s*(?:[들써맞]|로(?!리))|면도날|동맥|자해|목숨|세상\s*[뜨떠]|저\s*세상|먼저\s*[가갈]|따라\s*[가갈]|곁으로|조용히\s*[가갈떠]|떠날|떠나려|떠나야|하직|없어야|없으면\s*(?:편|낫|좋)|나\s*없으면|가\s*[뿌뿔삐불]|살\s*맛|폐[\s가-힣]{0,4}끼|짐\s*(?:만|이|덩)|그만\s*살|살기\s*(?:가|를)?\s*싫|사라[지져]|짐[\s가-힣]{0,6}정리|빠져\s*주|살아야\s*할\s*(?:까닭|이유)|살아갈\s*(?:이유|까닭)|살\s*(?:까닭|이유)\s*(?:가|이)?\s*없|살아서\s*[뭐뭣]|마지막일|잘\s*있으[란라]|이만\s*[가갈]|이만\s*정리|나눠\s*[줬줄주]|갈\s*때가\s*[됐되왔]|잘\s*지내[라요]|그동안\s*고마|더는\s*안\s*[봐볼]|없어졌으면|없어져|없어지고|눈\s*감|눈\s*안\s*[뜨떠떴]|안\s*깨|깨어나|영원|편하게\s*[가갈]|편히\s*[가갈]|(?:죽는|가는|편한)\s*방법|고통\s*없이|갈\s*수\s*있는\s*방법|끝내\s*(?:[버불뿔삐]|고\s*싶)|끝장|못\s*견디|고통스러|모[아았]|털어|들이[켜키]|삼[키켜켰]|먹어버|먹어불|입에\s*[넣털]|두\s*번|두\s*알|세\s*알|한\s*알|또\s*먹|또\s*한\s*번|한\s*번\s*더|안\s*먹은\s*줄|섞어|(?:약|먹|복용)[\s가-힣]{0,10}(?:헷갈|착각)|(?:헷갈|착각)[\s가-힣]{0,12}(?:약|먹|복용|알|번)|움큼|한\s*통[\s가-힣]{0,5}(?:먹|삼|들이|털)|숨[\s이가을]{0,2}(?:차|막|가[쁘뻐]|벅차|넘어가|안\s*쉬|못\s*쉬|쉬기\s*(?:가\s*)?(?:힘|어려))|가슴[\s이가을]{0,3}(?:답답|아[프파]|조여|뻐근|쥐어|짓눌|통증|뻑뻑|벌렁|두근)|쓰러[지졌진뜨]|넘어[지졌진]|미끄러[지졌진]|자빠|나자빠|삐끗|엎어|고꾸라|못\s*일어|일어나지\s*(?:를\s*)?못|일어날\s*수가?\s*없|주저앉|폭삭|허리를?\s*못\s*[쓰써]|(?<![가-힣])피[가\s]|출혈|어지(?:러|럼|럽|뱅)|핑\s*도|식은\s*땀|손\s*(?:이|을)?\s*떨|떨려|떨린|마비|경련|발작|거품|뻣뻣|떨면서|경기들|의식|정신이\s*[흐혼아가]|캄캄|아득|혼미|안\s*올라가|헛나|어눌|혀\s*꼬|부들부들|벌벌\s*떨|덜덜\s*떨|거품\s*물|까무러|까무라|기절|혼절|삐[뚤뚜]|비뚤|감각\s*(?:이|도)?\s*(?:없|둔)|(?:한|왼|오른|반)\s*[쪽짝편]\s*(?:팔|다리|손|발|몸|얼굴|눈|입)|반신\s*마|토[하해]|구역|메스|데[였여어]|화상|끓는|뜨거운\s*물|물집|살갗|부러[지졌진뜨]|골절|삐[었어]/;
