...
const PAST_CONTEXT_GUARD = /(예전에|옛날에|어릴\s*[때적]|어렸을\s*적|젊었을\s*[때적]|젊을\s*[적때]|젊은\s*시절|한창\s*때|왕년에|작년에|지난번에|저번에|꿈에서|꿈\s*꿨|TV|드라마|영화\s*(?:에서|봤|보고)|뉴스|방송에서)/;
...
const L3_RULES: PatternRule[] = [
  ...
  { level: 3, category: "medical_acute", pattern: /의식이[\s\S]{0,6}(?:흐|혼|없)|정신이[\s\S]{0,7}(?:흐려|혼미|아득|가물)|쓰러질\s*것\s*(?:만\s*)?같아/ },
