...
const PAST_CONTEXT_GUARD = /(예전에|옛날에|어릴\s*[때적]|어렸을\s*적|젊었을\s*[때적]|젊을\s*[적때]|젊은\s*시절|한창\s*때|왕년에|작년에|지난번에|저번에|꿈에서|꿈\s*꿨|TV\s*에서|드라마에서|영화에서|뉴스에서)/;
...
const MEDICATION_UNDERDOSE_GUARD = /(?:두\s*알|세\s*알|두\s*번|세\s*번)\s*(?:먹는\s*걸|먹어야|드시는\s*걸|복용해야|복용하는\s*걸)[^.]{0,15}(?:하나만|반만|덜|안\s*먹|빼먹|건너|잊|까먹|놓쳤|놓치)|약\s*(?:을|이)?\s*(?:빼먹|안\s*먹|덜\s*먹|건너|잊|까먹|놓쳤)/;
...
const L3_RULES: PatternRule[] = [
  ...
  { level: 3, category: "suicidal", pattern: /(?:정말|진짜|이제|그냥|다)?\s*죽고\s*싶|목(?:이라도|을|에|아)?\s*매(?:달|려|어|고\s*싶|아|서)|빨랫?줄에?\s*목|뛰어\s*내[리려릴]|투신|자살\s*(?:하|할|방법)|(?:다|그냥|이제|인자|모든\s*걸)\s*끝내(?:버리|뿔|삐|불|고\s*싶|려|뻔)|사라(?:지|져)\s*(?:고\s*싶|버리|버려|면\s*좋겠)|없어져\s*버리(?:고\s*싶|면)|살기\s*(?:가|를)?\s*싫|살\s*맛(?:이|도)?\s*안|그만\s*살(?:고\s*싶|아야|아|래)|이\s*세상에서?\s*없(?:어지|어지고)|살아갈\s*이유\s*없|살아서\s*뭐|살아\s*뭐\s*해|짐(?:만|이|이만)\s*(?:되|되는|되겠|돼|되네|되니|되어)|폐(?:만|를)\s*(?:끼치|끼친|되|돼)|빨리\s*(?:가야|가버려|떠나야|죽어야)|어서\s*가야|먼저\s*(?:가야|가버려|떠나야|죽으면|가는\s*게)|얼른\s*죽어/ },
];
  ...
  for (const rule of L3_RULES) {
    ...
    if (m) {
      ...
      if (isPastContext) {
        ...
        if (rule.category === "medication_error" && (/적\s*(?:이|도)?\s*있었|그\s*뒤로|그\s*후로?|이제[는요]?\s*(?:잘|괜찮)|인자[는]?\s*(?:잘|괜찮|챙)|지금[은]?\s*(?:잘|괜찮)/.test(text) || /뉴스에서|드라마에서|TV\s*에서|영화에서|방송에서|어떤\s*(?:사람|노인|할[머아])/.test(text))) continue;
