...
const L3_RULES: PatternRule[] = [
  ...
  { level: 3, category: "suicidal", pattern: /(?:약|수면제)(?:을|를|이|들)?[\s\S]{0,18}(?:먹|복용|삼[키켜켰]|털어\s*넣|입에\s*털|들이[켜키])[\s\S]{0,18}(?:죽|자\s*버리|자버리|잠들|영원히|안\s*깨|깨어나지\s*않|깨지\s*않|못\s*깨|다신?\s*안|끝(?:내|장)|따라가|가\s*버리|가\s*뿌|확\s*[가갈갔간]|조용히\s*[가갈갔간]|먼저\s*[가갈갔간]|눈\s*(?:안\s*)?[뜨떠떴]|눈\s*감|저\s*세상|세상\s*(?:뜨|떠))|(?:죽으려|죽을라|따라가|끝내려|영원히\s*잠들|눈\s*안\s*[뜨떠떴]|조용히\s*[가갈]|확\s*[가갈])[\s\S]{0,22}(?:약|수면제)(?:을|를|이)?[\s\S]{0,12}(?:먹|복용|삼[키켜켰]|털어\s*넣|다\s*먹)|(?:약|수면제)[\s\S]{0,15}(?:모아|모았|차곡)[\s\S]{0,15}(?:먹|복용|털어)/ },
  // 약 오용 — 다양한 어순·표현 (조사·키워드 위치 자유)
  //   2026-06-25 적대적 검증: 조사(두 알이나/씩)·섞어서 먹·깜빡하고 또 변형 누락 메움.
  { level: 3, category: "medication_error", pattern: /약\s*(?:을|이|를)?\s*(?:잘못|많이|두\s*번|세\s*번|이중으로|이중|두\s*알|세\s*알|여러\s*알|여러\s*번|또|한\s*번\s*더|다시)(?:\s*(?:이나|씩|을|를|더))*\s*(?:먹|복용|드|삼|넣|치)|약\s*(?:을|이|를)?\s*(?:먹은\s*것\s*같|먹었던\s*것\s*같|복용했던\s*것\s*같)\s*(?:은\s*데|아|어)?\s*(?:또|다시|한\s*번\s*더)|(?:또|다시|한\s*번\s*더|두\s*번)\s*약\s*(?:을|이|를)?\s*(?:먹|복용|드|삼)|(?:깜빡|까먹)(?:고|어서|어\s*가지고|해서|하고)?\s*(?:또\s*)?약\s*(?:을|이|를)?[\s\S]{0,8}(?:또|다시|한\s*번\s*더)?\s*(?:먹|복용|드)|약\s*(?:을|이|를)?[\s\S]{0,10}(?:깜빡|까먹)[\s\S]{0,8}(?:또|다시|한\s*번\s*더)\s*(?:먹|복용|드)|약\s*(?:을|이|를)?\s*먹(?:고|었는데|었어|었더니|었는|었어요|었지)[\s\S]{0,30}(?:또|다시|한\s*번\s*더|한\s*알\s*더|두\s*번|두\s*알)(?:\s*(?:이나|씩|더))*\s*(?:먹|복용|드|삼)|(?:한\s*알|두\s*알|세\s*알)(?:\s*(?:이나|씩|을|를))*\s*더\s*(?:먹|복용|드|삼)|약[\s\S]{0,12}섞어[\s\S]{0,6}(?:먹|복용|드)|(?:헷갈|착각)[\s\S]{0,12}(?:두\s*번|또|다시|두\s*알|한\s*번\s*더)\s*(?:먹|복용|드)|약\s*과다|수면제\s*(?:을|를|이)?\s*(?:많이|여러\s*알|두\s*알|세\s*알|두\s*번|한\s*통)/ },
  ...
  { level: 3, category: "suicidal", pattern: /(?:정말|진짜|이제|그냥|다)?\s*죽고\s*싶|목\s*매(?:달|려|어)|뛰어내리|자살\s*(?:하|할|방법)|(?:다|그냥|이제|모든\s*걸)\s*끝내(?:버리|고\s*싶|려)|사라(?:지|져)\s*(?:고\s*싶|버리|버려|면\s*좋겠)|없어져\s*버리(?:고\s*싶|면)|살기\s*싫|그만\s*살(?:고\s*싶|아야|아|래)|이\s*세상에서?\s*없(?:어지|어지고)|살아갈\s*이유\s*없|살아서\s*뭐|살아\s*뭐\s*해|짐(?:만|이|이만)\s*(?:되|되는|되겠|돼|되네|되니|되어)|폐(?:만|를)\s*(?:끼치|끼친|되|돼)|빨리\s*(?:가야|가버려|떠나야|죽어야)|어서\s*가야|먼저\s*(?:가야|가버려|떠나야|죽으면|가는\s*게)|얼른\s*죽어/ },
];
