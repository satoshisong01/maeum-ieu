...
const L3_RULES: PatternRule[] = [
  ...
  { level: 3, category: "suicidal", pattern: /(?:약|수면제)(?:을|를|이)?[\s\S]{0,15}(?:먹|복용|삼[키켜])[\s\S]{0,20}(?:죽|자\s*버리|자버리|잠들[면어]|영원히|안\s*깨|다신?\s*안\s*깨|끝(?:내|장)|따라가)|(?:죽으려|따라가려|끝내려)[\s\S]{0,15}(?:약|수면제)[\s\S]{0,10}(?:먹|복용)/ },
  // 약 오용 — 다양한 어순·표현 (조사·키워드 위치 자유)
  { level: 3, category: "medication_error", pattern: /약\s*(?:을|이)?\s*(?:잘못|많이|두\s*번|세\s*번|이중으로|이중|섞어|두\s*알|세\s*알|여러\s*번|또|한\s*번\s*더|다시)\s*(?:먹|복용|드|삼|넣)|약\s*(?:먹은\s*것\s*같|먹었던\s*것\s*같|복용했던\s*것\s*같)\s*(?:은\s*데|아|어)?\s*(?:또|다시|한\s*번\s*더)|(?:또|다시|한\s*번\s*더|두\s*번)\s*약\s*(?:을|이)?\s*(?:먹|복용|드|삼)|(?:깜빡|까먹)(?:고|어서|어\s*가지고)?\s*약\s*(?:을|이)?\s*(?:또|다시|한\s*번\s*더)?\s*(?:먹|복용|드)|약\s*먹(?:고|었는데|었어|었더니|었는|었어요)[\s\S]{0,30}(?:또|다시|한\s*번\s*더|한\s*알\s*더|두\s*번|두\s*알)\s*(?:먹|복용|드|삼)|(?:한\s*알|두\s*알|세\s*알)\s*더\s*(?:먹|복용|드|삼)|약\s*과다|수면제\s*(?:많이|여러\s*알|두\s*알|두\s*번)/ },
