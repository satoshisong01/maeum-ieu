...
const L3_RULES: PatternRule[] = [
  ...
  // 약 오용 — "약 두 번/약 잘못/약 많이/약 이중" 패턴 (조사 유무 모두). "약 두 번 먹은 것 같아" 케이스 보강.
  { level: 3, category: "medication_error", pattern: /약\s*(?:을|이)?\s*(?:잘못|많이|두\s*번|세\s*번|이중으로|이중|섞어|두\s*알|세\s*알|여러\s*번|또)\s*(?:먹|복용|드|삼|넣)|약\s*(?:먹은\s*것\s*같|먹었던\s*것\s*같|복용했던\s*것\s*같)\s*(?:은\s*데|아|어)?\s*(?:또|다시|한\s*번\s*더)|약\s*과다|수면제\s*(?:많이|여러\s*알|두\s*알|두\s*번)/ },
