...
console.log("\n[A-5] 백스톱 사전필터(SOFT_SIGNAL) 커버리지");
{
  ...
  for (const t of ["이 끈으로 목이라도 매달까", "그 양반 곁으로 따라갈라고", "다시는 안 깨어났으면", "조용히 눈 안 뜨고 가는", "살 맛이 안 나여", "끓는 물에 살갗이 다 익고 물집이", "또 한 알 더 입에 넣었구먼", "식은땀 나고 손이 떨려", "폐만 끼치는 것 같아", "아무한테도 폐 안 끼치고 조용히 가뿌고 싶구마"])
    check(`사전필터 통과(우려): ${t}`, SOFT_SIGNAL.test(t));
  // 순수 일상은 사전필터에 안 걸려 백스톱 skip(비용 억제)
  for (const t of ["오늘 날씨가 참 좋네", "손주랑 공원 다녀왔어", "된장찌개 먹었지 맛있더라", "경로당에서 화투 쳤어"])
