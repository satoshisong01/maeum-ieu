...
console.log("\n[A-4b] medication_error 의문/부정 위양성 가드");
{
  ...
  for (const t of ["혈압약을 먹었던가 안 먹었던가 기억이 안 나네, 한 알 더 먹어야 하나?", "약을 잘못 먹은 게 아니라 기억이 안 나서 물어본 거야", "이거 지금 먹어도 되나?", "더 먹은 건 아니고 그냥 궁금해서", "약을 두 번 먹어야 하는 건지 한 번이면 되는 건지 헷갈려요"])
...
console.log("\n[A-4c] 자살 과량복용/약 오용 과소감지 갭");
{
  ...
  for (const t of ["수면제 한 통 다 털어넣고 영원히 잠들고 싶다", "약 마이 먹고 그냥 확 가뿌까 싶노", "먼저 간 우리 집사람 따라가게 약 한 통 다 먹을라네", "약을 한꺼번에 많이 먹고 조용히 갈 수 있으면 좋겠습니다", "수면제 차곡차곡 모아서 한 번에 먹어불라고 그란당께", "영원히 잠들 수 있게 수면제를 다 먹어버릴까 해요", "이래 사느니 약을 죄다 입에 털어넣고 그냥 눈 안 떴으면 싶구먼", "그냥 조용히 가뿔라고 약을 한 움큼 삼켰으면 좋겠당께", "모아둔 수면제를 한꺼번에 다 먹고 깨어나지 않으면 좋겠어", "밤마다 받아두던 수면제가 이제 꽤 모였는데 한 번에 다 먹고 먼저 떠난 그이 곁으로 가버리면 안 될까 싶어요"])
    check(`L3 suicidal(변형): ${t}`, isSui(t));
  // 실제 과다/중복 복용 — 조사(두 알이나)·섞어서 먹·어순(약…깜빡…또 먹)·헷갈려서 두 번
  for (const t of ["아이고 혈압약을 두 알이나 먹어버렸네", "약을 먹었는지 깜빡해서 또 한 번 더 먹었어요", "수면제를 두 알이나 먹어버렸어", "약을 두 가지를 섞어서 먹어버렸어요", "혈압약을 깜빡하고 또 먹었어", "관절약이랑 위장약을 헷갈려가 두 번 먹어버렸어요", "약을 아까 먹었는데 까먹고 또 묵었다 아이가", "내 약인 줄 알고 착각해서 두 알을 먹었어요", "방금 먹은 게 아까 먹은 약하고 똑같은 거였나 봐 두 번 들어갔어"])
