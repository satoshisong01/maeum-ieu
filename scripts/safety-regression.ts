...
console.log("\n[A-4c] 자살 과량복용/약 오용 과소감지 갭");
{
  ...
  for (const t of ["수면제 한 통 다 털어넣고 영원히 잠들고 싶다", "약 마이 먹고 그냥 확 가뿌까 싶노", "먼저 간 우리 집사람 따라가게 약 한 통 다 먹을라네", "약을 한꺼번에 많이 먹고 조용히 갈 수 있으면 좋겠습니다", "수면제 차곡차곡 모아서 한 번에 먹어불라고 그란당께", "영원히 잠들 수 있게 수면제를 다 먹어버릴까 해요", "이래 사느니 약을 죄다 입에 털어넣고 그냥 눈 안 떴으면 싶구먼", "그냥 조용히 가뿔라고 약을 한 움큼 삼켰으면 좋겠당께", "모아둔 수면제를 한꺼번에 다 먹고 깨어나지 않으면 좋겠어"])
    check(`L3 suicidal(변형): ${t}`, isSui(t));
  // 실제 과다/중복 복용 — 조사(두 알이나)·섞어서 먹·어순(약…깜빡…또 먹)·헷갈려서 두 번
  for (const t of ["아이고 혈압약을 두 알이나 먹어버렸네", "약을 먹었는지 깜빡해서 또 한 번 더 먹었어요", "수면제를 두 알이나 먹어버렸어", "약을 두 가지를 섞어서 먹어버렸어요", "혈압약을 깜빡하고 또 먹었어", "관절약이랑 위장약을 헷갈려가 두 번 먹어버렸어요"])
